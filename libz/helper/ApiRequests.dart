...
import 'package:events/helper/Models/CountryModel.dart';
import 'package:events/helper/Models/FavouriteModel.dart';
import 'package:events/helper/Models/NotifcationModel.dart';
import 'package:events/helper/Models/RegesterModel.dart';
import 'package:events/helper/Models/SearchModel.dart';
import 'package:events/helper/Models/searchResult.dart';
...
class ApiRequests {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData["msg"] == "success") {
          ...
          pref.setString(LAT, upaderesponse.data.lat.toString());
          pref.setString(LONG, upaderesponse.data.lng.toString());
          pref.setString(MobileNumber, upaderesponse.data.phone.toString());
          pref.setString(Address, upaderesponse.data.address.toString());
          
          _dataUser.setKey(
              {'key': Name, 'value': upaderesponse.data.fullName.toString()});
          _dataUser
              .setKey({'key': LAT, 'value': upaderesponse.data.lat.toString()});
          _dataUser.setKey(
              {'key': LONG, 'value': upaderesponse.data.lng.toString()});
          _dataUser.setKey({
            'key': MobileNumber,
            'value': upaderesponse.data.phone.toString()
          });
          _dataUser.setKey(
              {'key': Address, 'value': upaderesponse.data.address.toString()});
  ...
  Future<bool> editeProfile(String name, String phone, String lat,String long ,String address,String email,File imagefile) async {
    SharedPreferences pref = await SharedPreferences.getInstance();
    String token = pref.getString(Token);
    //String email = pref.getString(Utility.Email);
    print("hhhhhhhhhhhhhhhhhhhhh");
    ...
    formData.fields..add(MapEntry('lng', long));
    formData.fields..add(MapEntry('address', address));
...
Future<SearchMModel> getsearchdata() async {
 ...
 Future<SearchResult> getsearch(String catid,String countryid,String cityid,String startdate,String enddate) async {
    print("get joinde room function ");
    SharedPreferences pref = await SharedPreferences.getInstance();
    String token = pref.getString(Token);
