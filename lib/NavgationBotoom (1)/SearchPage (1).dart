...
import 'package:dailyevent/NavgationBotoom%20(1)/tasnefatpage2%20(1).dart';
...
import 'SearchCountry (1).dart';
import 'searchCatogry (1).dart';
import 'searchcity (1).dart';
...
class _SearchPageState extends State<SearchPage> {
                                ...
                                onTap: () {
                                  if (searchcontyitem != null) {
                                    Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                            builder: (context) => DoctorSearch2(
                                                data: searchcontyitem))).then(
                                        (chooseProduct) {
                                      // print("-product in home");
                                      // print(chooseProduct['DoctorName']);
                                      // print(chooseProduct['DoctorId']);
                              ...
                              InkWell(
                                onTap: () {
                                  Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                          builder: (context) => DoctorSearch23(
                                              data:
                                                  _data.data.categories))).then(
                                      (chooseProduct) {
                                    // print("-product in home");
                                    // print(chooseProduct['DoctorName']);
                                    // print(chooseProduct['DoctorId']);
                                    if (chooseProduct.toString() != "null") {
                                      setState(() {
                                        categoryname =
                                            chooseProduct['DoctorName']
                                                    .toString() ??
  ...
  void validate() {
    ...
    if ((!(time == null || time == "اختر تاريخ البداية")) &&
        (!(time2 == null || time2 == "اختر تاريخ النهاية"))) {
      print("mahmoud");
      if (end.isBefore(start)) {
        CommnWidget.showInSnackBar("ادخل تاريخ البداية و النهاية بطريقة صحيحة",
    ...
    _apiRequests
      ...
      if (value.data.length == 0) {
        CommnWidget.showInSnackBar(
            "لا يوجد فعاليات", CommnWidget.commonColor, _scaffoldKey);
        return;
      }
