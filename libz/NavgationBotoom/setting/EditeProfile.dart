...
import 'package:events/NavgationBotoom/setting/changPassword.dart';
            ...
            children: <Widget>[
                    ...
                    child: InkWell(
                      onTap: (){
                          ...
                          errorWidget: (context, url, error) =>
                              Icon(Icons.error, color: Colors.black)):Image.file(
                                            _image,
                                            fit: BoxFit.cover,
                                            width: 100,
                                            height: 100,
              ...
              Align(
                ...
                child: Container(
                  ...
                  child: InkWell(
                    onTap: () async {
                      LocationResult result = await LocationPicker.pickLocation(context, "AIzaSyBQFncM9uaq-G4j0fb3nMWTRdbzwODGHzM");
                      //  showLocationPicker(
...
SizedBox(height:MediaQuery.of(context).size.height*.1,),
               ...
               child: CupertinoActivityIndicator(
                    ...
                    child: CommnWidget.commnbtn("تعديل", context))),
  ...
  Future getImage(ImageSource imageSource) async {
    ...
    if (_formKey.currentState.validate()) {
      ...
      if (nameContrller.text == null||nameContrller.text.isEmpty) {
        ...
        CommnWidget.showInSnackBar(
            "رجاء ادخل البريج الاكتروني", CommnWidget.commonColor, _scaffoldKey);
        return;
      }
                  if (phoneContrller.text == null||phoneContrller.text.isEmpty) {
        CommnWidget.showInSnackBar(
            "رجاء ادخل البريد الاكتروني", CommnWidget.commonColor, _scaffoldKey);
        return;
      }
     setState(() {
        _isLoading=true;
      ...
      _apiRequests.editeProfile(nameContrller.text, phoneContrller.text, lat, long, locationContrller.text, emailContrller.text, _image) .then((value){
     if(value!=null){
if(value){
   _apiRequests.getProfile().then((value){
if(value==true){
 CommnWidget.showErrorDialog("حسنا ","تم تعديل بياناتك بنجاح", DialogType.SUCCES, context);
      setState(() {
        _isLoading=false;
      });
}else if(value == false){
