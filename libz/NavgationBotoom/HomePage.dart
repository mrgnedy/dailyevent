...
import 'package:events/helper/Models/HomeModel.dart';
...
import 'package:events/tasnefatepage.dart';
import 'package:events/widget/EventsWidget.dart';
    ...
    return Scaffold(
      key: _scaffoldKey,
        appBar: AppBar(
          ...
          actions: <Widget>[
            IconButton(
              ...
              onPressed: () {
         widget.isloading==false?   CommnWidget.showInSnackBar2(" قم بتسجيل الدخول اولا", CommnWidget.commonColor, _scaffoldKey,context)    : Navigator.push(context, MaterialPageRoute(builder: (context) {
        ...
        margin: EdgeInsets.only(right: 8, left: 8,),
                          child:_data.data.ads.length==0?Center(child: Text("لا يوجد بيانات في الوقت الحالي",style: TextStyle(fontFamily: "thin"),)) :Swiper(
                            autoplay: true,
                        ...
                        Container(
                          ...
                          width: MediaQuery.of(context).size.width,
                          margin: EdgeInsets.only(right: 8, left: 8,),
                          child:_data.data.todayevents.length==0?Center(child: Text("لا يوجد بيانات في الوقت الحالي",style: TextStyle(fontFamily: "thin"),)) :Swiper(
                            autoplay: true,

                            itemBuilder: (BuildContext context, int index) {
                              id = _data.data.todayevents[index].id.toString();
                              return InkWell(
                                onTap: () {
                                  Navigator.push(context,
                                      MaterialPageRoute(builder: (context) {
                                    return TasnefatDetails(
                                      data: _data.data.todayevents[index],
                                    );
                                  }));
                                },
                                child: ClipRRect(
                                      borderRadius: BorderRadius.circular(12),
                                                                  ...
                                                                  child: CachedNetworkImage(
                                      imageUrl: IMAGEURL +
                                          _data.data.todayevents[index].image
                                              .toString(),
                                      placeholder: (context, url) => Center(
                                            child: CupertinoActivityIndicator(
                                              radius: 17,
                    ...
                    _data.data.todayevents.length==0?Container():    InkWell(
                          onTap: (){
                            widget.isloading==false? CommnWidget.showInSnackBar2("تاكد من الاتصال بالانترنت", CommnWidget.commonColor, _scaffoldKey,context):
                            _apiRequests.setfavorite(id).then((value){
if(value==null){
  CommnWidget.showInSnackBar("تاكد من الاتصال بالانترنت", CommnWidget.commonColor, _scaffoldKey);
}else{
  if(value){
  CommnWidget.showInSnackBar("تم اضافة هذه الفاعلية الي المفضلة", CommnWidget.commonColor, _scaffoldKey);
  }else{
     CommnWidget.showInSnackBar("هذه الفاعلية مضافة سابقا ", CommnWidget.commonColor, _scaffoldKey);
  }
}
                            });
                          },
...
return TasnefatPage2(data: _data.data.recommendevents,);
}));
                                          },
                                                                                  child: Text(
                                            "عرض الجميع",
                                              ...
                                              return InkWell(
                                                ...
                                                child: ChoosenEventCell(
                                                  ...
                                                  location: _data.data
                                                      ...
                                                      .toString(),
                                                       lastdate:DateTime.parse( _data.data.recommendevents [i].endDate.toString()) 
                                                ),
                                              );
                                            },
                                          ),
